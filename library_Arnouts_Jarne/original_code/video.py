...
class VideoAnalysisObserver(Observer):
    ...
    def __init__(self, panel, observable):
        self.observable = observable
        self.panel = panel
        self.analysis = False
        self.tello_pose_analysis = Tello_Pose()
    ...
    def on_next(self, value):
        if self.analysis:
            ...
            cmd, self.draw_skeleton, self.points = self.tello_pose_analysis.detect(frame)
            if cmd != '':
...
class VideoMonitor(object):
    ...
    def change_analysis(self):
        self.video_analysis_observer.analysis = not self.video_analysis_observer.analysis
        self.basic_video_observer.show = not self.basic_video_observer.show

    def change_pose_commands(self, change):
        self.video_analysis_observer.tello_pose_analysis.change_pose_command(change)
