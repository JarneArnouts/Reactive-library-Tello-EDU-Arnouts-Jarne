...
class Tello_Pose:
    def __init__(self):
        ...
        self.protoPath = join(dirname(__file__), "model/pose/mpi/pose_deploy_linevec_faster_4_stages.prototxt")
        self.modelPath = join(dirname(__file__), "model/pose/mpi/pose_iter_160000.caffemodel")
        ...
        # new poses
        self.arm_up_45_cnt = 0  # count numbers of the arm_up45 captured in every certain number of frames
        self.arm_up_straight_cnt = 0  # count numbers of the arm_upS captured in every certain number of frames
        self.arm_down_straight_cnt = 0  # count numbers of the arm_downS captured in every certain number of frames
        ...
        # dictionary with pose-command combinations
        self.pose_dict = {'Flat': "takeoff",
                          'Down': "down",
                          'V': "flipl",
                          'Up': "up",
    ...
    def change_pose_command(self, change):
        pose = change[0]
        command = change[1]
        self.pose_dict[pose] = command
    ...
    def is_arms_up_straight(self, points):
        """
        Determine if the person is holding  the arms
        like:
                |
    ...
    def is_arms_down_straight(self, points):
        """
        Determine if the person is holding  the arms
        like:
                |
    ...
    def detect(self, frame):
        ...
        if self.is_arms_down_45(points):
            ...
            print("%d:arm down 45 captured" % self.frame_cnt)

        if self.is_arms_flat(points):
            self.arm_flat_cnt += 1
            print("%d:arm flat captured" % self.frame_cnt)
        ...
        # new poses check
        if self.is_arms_up_45(points):
            self.arm_up_45_cnt += 1
            print("%d:arm up 45 captured" % self.frame_cnt)

        if self.is_arms_up_straight(points):
            self.arm_up_straight_cnt += 1
            print("%d:arm up straight captured" % self.frame_cnt)

        if self.is_arms_down_straight(points):
            self.arm_down_straight_cnt += 1
            print("%d:arm down straight captured" % self.frame_cnt)
        ...
        if self.frame_cnt == frame_cnt_threshold:
            if self.arm_down_45_cnt >= pose_captured_threshold:
                print('!!!arm down!!!')
                cmd = self.pose_dict['Down']
            if self.arm_flat_cnt >= pose_captured_threshold:
                print('!!!arm flat!!!')
                cmd = self.pose_dict['Flat']
            if self.arm_V_cnt == self.frame_cnt:
                print('!!!arm V!!!')
                cmd = self.pose_dict['V']
            if self.arm_up_45_cnt == self.frame_cnt:
                print('!!!arm up 45!!!')
                cmd = self.pose_dict['Up']
            if self.arm_up_straight_cnt == self.frame_cnt:
                print('!!!arm up straight!!!')
                cmd = self.pose_dict['UpStraight']
            if self.arm_down_straight_cnt == self.frame_cnt:
                print('!!!arm down straight!!!')
                cmd = self.pose_dict['DownStraight']
            self.frame_cnt = 0
